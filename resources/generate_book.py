...
def make_book(rm: ResourceManager, i18n: I18n, local_instance: bool = False, reverse_translate: bool = False):
    ...
    book.category('firmalife', 'Firmalife', 'All about the Firmalife addon', 'firmalife:brick_oven_top', is_sorted=True, entries=(
        ...
        entry('cheese', 'Cheese', 'firmalife:textures/item/food/gouda.png', pages=(
            ...
            text('Making curds leaves $(thing)Whey$() behind in the barrel. Seal the whey with a $(thing)Cheesecloth$() to make $(thing)Ricotta$().'),
            text('$(thing)Mozzarella$() is made without rennet. Pour $(thing)Vinegar$() into a barrel of milk to curdle it into $(thing)Milk Vinegar$(), then seal that with a $(thing)Cheesecloth$().'),
            text('$(thing)Blue Cheese$() needs $(thing)Blue Mold$(). Culture it by sealing $(thing)rotten bread$() and water in a barrel, which yields four mold.'),
            crafting('firmalife:crafting/blue_wheel', text_contents='Make $(thing)Blue Cheese$() by crafting $(thing)Milk Curd$() with $(thing)Salt$() and $(thing)Blue Mold$(), or by $(item)$(k:key.use)$() Blue Mold on any fresh cheese wheel.'),
        )),
        entry('climate_station', 'Climate Station', 'firmalife:climate_station', pages=(
            ...
            text('$(li)It updates periodically on its own, or when placed/broken.$()$(li)When a climate station updates, it tells all the blocks inside the multiblock that they can operate. For example, it lets $(l:firmalife/cheese#cheesecloth)Cheese$() begin aging.$()$(li)Press $(item)$(k:key.use)$() to force update the Climate Station and the blocks inside the multiblock.', 'Climate Station Tips'),
        ...
        entry('beekeeping', 'Beekeeping', 'firmalife:beehive', pages=(
            ...
            text('Bees, in general, are active above 7°C, though hardier species tolerate colder temperatures. A $(thing)Wild Beehive$() will always contain bees upon world generation, and will not lose the bees unless disturbed. If time has passed and the bees are active, the wild beehive will show that it is dripping with honey.'),
        ...
        entry('beekeeping_reference', 'Beekeeping Reference', 'firmalife:textures/item/queen_bee.png', pages=(
            ...
            text('Using $(thing)Blue Mold$() (from $(l:firmalife/cheese)cheesemaking$()) on a hive cures brood infections. It works on all parasitic infections except $(thing)Varroa$(), which must be escaped by swarming.'),
            text('Refer to the next pages for information on bee species.'),
            ...
            text('$(li)$(thing)Western Honey Bee$(): Calmness, Production, Hardiness. 5-20°C, 100-400mm. Extremely common. Western bees also spawn when no other bee species is eligible to spawn.$()$(li)$(thing)Asian Honey Bee$(): Fertility, Nature Restoration. 10-30°C, 250-400mm. Common.$()$(li)$(thing)Giant Honey Bee$(): Fertility. 15-30°C, 300-450mm. Common.$()'),
        ...
        entry('ovens', 'Ovens', 'firmalife:brick_oven_top', pages=(
            ...
            multimultiblock('An example oven structure, uncured and cured.', *[multiblock('', '', True, (
                ...
                '0': 'firmalife:%soven_top[facing=west]' % pref,
                'T': 'firmalife:%soven_top[facing=west]' % pref,
                'B': 'firmalife:%soven_bottom[facing=west]' % pref,
                'W': 'minecraft:bricks',
        ...
        entry('wine', 'Winemaking', 'firmalife:textures/item/food/white_grapes.png', pages=(
            ...
            text('To use a stomping barrel, $(item)$(k:key.use)$() with fresh grapes. Then, jump up and down on the barrel 16 times. $(item)$(k:key.use)$() with an empty hand to retrieve the items.$(br2)Then, seal the grapes in a barrel of water for 5 days to $(thing)ferment$() them.'),
        ...
        entry('pie', 'Pie', 'firmalife:textures/item/food/cooked_pie.png', pages=(
            ...
            text('$(thing)Butter is the base of many such foods. Using a $(l:firmalife/mixing_bowl)Mixing Bowl$(), combine 1000 mB of $(thing)Cream$() with one item of $(thing)Salt$(). Cream is made by sealing 1000 mB of milk in a barrel with a $(l:firmalife/cheese#cheesecloth)Cheesecloth$().'),
