...
public final class FLFruitBlocks
{
    public enum Tree
    {
        COCOA(FLItems.FOODS.get(FLFood.COCOA_BEANS), new Color(252, 171, 255).getRGB(), new Lifecycle[] {HEALTHY, HEALTHY, HEALTHY, FLOWERING, FLOWERING, FRUITING, DORMANT, DORMANT, DORMANT, DORMANT, DORMANT, HEALTHY}),
        FIG(FLItems.FRUITS.get(FLFruit.FIG), new Color(251, 135, 255).getRGB(), new Lifecycle[] {HEALTHY, HEALTHY, FLOWERING, FLOWERING, FRUITING, DORMANT, DORMANT, DORMANT, DORMANT, DORMANT, HEALTHY, HEALTHY});
        ...
        {
            return new FLFruitTreeSaplingBlock(ExtendedProperties.of().noCollission().randomTicks().strength(0).sound(SoundType.GRASS).blockEntity(FLBlockEntities.TICKING_PLANT).flammableLikeLeaves(), FLBlocks.FRUIT_TREE_GROWING_BRANCHES.get(this), 8 * ICalendar.CALENDAR_TICKS_IN_DAY, FLClimateRanges.FRUIT_TREES.get(this), stages);
        }

        public Block createPottedSapling()
        {
            return new FlowerPotBlock(() -> (FlowerPotBlock) Blocks.FLOWER_POT, FLBlocks.FRUIT_TREE_SAPLINGS.get(this), BlockBehaviour.Properties.of().instabreak().noOcclusion());
        }
        ...
        {
            return new FruitTreeBranchBlock(ExtendedProperties.of().mapColor(MapColor.WOOD).sound(SoundType.SCAFFOLDING).randomTicks().strength(1.0f).flammableLikeLogs(), FLClimateRanges.FRUIT_TREES.get(this));
        }

        public Block createGrowingBranch()
        {
            return new FLGrowingFruitTreeBranchBlock(ExtendedProperties.of().mapColor(MapColor.WOOD).sound(SoundType.SCAFFOLDING).randomTicks().strength(1.0f).blockEntity(FLBlockEntities.TICKING_PLANT).flammableLikeLogs(), FLBlocks.FRUIT_TREE_BRANCHES.get(this), FLBlocks.FRUIT_TREE_LEAVES.get(this), FLClimateRanges.FRUIT_TREES.get(this));
        }
    }
